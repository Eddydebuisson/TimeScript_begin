...
export default class Partie {
  ...
  game(nombreTours: number, nombreDe: number, ...joueurs: Joueur[]) {
    this.initialiserPartie(nombreTours,nombreDe,joueurs);
    this.lancerPartie();
    this.finDePartie();
  }
  ...
  finDePartie() {
    ...
    this._nombreJoueurs.forEach((joueur) => {
      if (maxPartiGagner < joueur._nombreDeTourGagner) {
        maxPartiGagner = joueur._nombreDeTourGagner
        joueurAvecMaxPartiGagner = joueur;
      }
      console.log(
        `Le joueur ${joueur._nom} a gagner ${joueur._nombreDeTourGagner} partie`
      );
    });
    console.log(
      `Le grand gagnant du jeu est !! **Roulement de tambour** ${joueurAvecMaxPartiGagner._nom}`
    );
  }
}
